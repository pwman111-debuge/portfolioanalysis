...
## ⚠️ 순매매 상위 테이블 칼럼 주의 (반복 오류 방지)

네이버 증권 **외국인/기관 순매매 상위** 페이지(`sise_deal_rank.naver`)는 두 날짜를 나란히 표시한다:
- **좌측 칼럼 = 전일** / **우측 칼럼 = 당일(오늘)** ← 반드시 우측 사용
- HTML 파싱 시 첫 번째 등장 블록이 전일이므로, 두 번째 날짜 헤더 이후 데이터를 사용한다.
- Failover로 해당 페이지 WebFetch 시에도 동일 원칙 적용.
